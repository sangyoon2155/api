...
public class CountryService {
	...
	private CityRepository cityuRepository;
	
	// 필드주입 대신 생성자 주입을 사용
	public CountryService(CountryRepository countryRepository, CityRepository cityRepository) {
		this.countryRepository = countryRepository;
		this.cityuRepository = cityRepository;
	}
	
	// country 삭제
	public boolean delete(int countryId) {
		// 이슈 : 자식테이블에 외래키로 참조하는 행이 있다면?
		// 자식테이블에 참조하는 행의 없다면 (select count(*) form city where country_id = ?)
		
		if(0 == cityuRepository.countByCountryEntity(countryRepository.findById(countryId).orElse(null))) {
